...
如果終端機已有另一個開發程序，請先按 `Control + C` 停止，再重新執行。

### 顯示 Port 5173 已被使用

代表已有 Vite 開發伺服器正在執行。回到先前的終端機按 `Control + C`，再重新啟動。
...
Version 1 使用固定 fixture 與元件內暫存狀態，尚未連接資料庫，因此重新啟動後會回到預設 Demo 情境。

### AI 為什麼不會回答其他問題？

目前使用 `MockAgentProvider`，只提供可重現的固定回答。正式 LLM、資料來源、引用與 Python Agent 將在 Version 2 補充。
...
- 資料庫與跨裝置同步
- 問卷到輪廓、規劃及評分的正式計算引擎
- Electron macOS／Windows 安裝包與簽章

Version 2 應沿用現有 `InvestmentRepository`、`AgentProvider` 與 shared contracts 邊界，避免 React 畫面直接依賴特定後端或模型供應商。

## 10. 免責聲明

本專案目前為競賽 Demo。畫面中的名稱、分數、事件、日期與金融內容均為固定假資料，不是即時資訊，不構成投資建議或買賣指令。
