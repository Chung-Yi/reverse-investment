...
export function AiDrawer({ open, route, initialPrompt, provider, onClose }: AiDrawerProps) {
  ...
  const submit = async () => {
    ...
    const request = await provider.submitMessage({ message: input, context: { route, screenTitle: routeMetadata[route].label } });
    ...
    <>
      ...
      <aside className={`ai-drawer ${open ? "open" : ""}`} aria-hidden={!open} aria-label="AI 對話助理">
        ...
        <div className="ai-body">
          <div className="ai-message"><span>AI</span><div><strong>{answer ? "示範回答" : "想先理解哪一部分？"}</strong><p>{answer || "我會承接目前畫面的脈絡。Version 1 使用固定回答，不會連線外部 AI。"}{running && <i className="typing-cursor" />}</p></div></div>
        ...
        <footer><label htmlFor="ai-input">輸入問題</label><div><input id="ai-input" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void submit(); }} placeholder="輸入你想了解的問題" /><button onClick={() => void submit()} disabled={running} aria-label="送出問題">↑</button></div><p>Demo AI 回答為固定假資料，正式版需附資料來源。</p></footer>
      </aside>
    </>
  );
}
