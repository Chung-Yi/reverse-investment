...
export function DecisionPage({ data }: { data: InvestmentData }) {
  ...
  const renderStep = () => {
    ...
    if (activeStep === 1) {
      return <div className="step-panel" role="tabpanel" aria-label="支持證據"><span className="card-label">Step 2｜支持證據</span><h2>哪些證據支持你的投資理由？</h2><p>以下承接標的分析頁的官方公開資料。請至少選擇一項真正與理由相關的證據。</p><div className="selection-list">{instrument.evidence.map((item) => <label className={selectedEvidence.includes(item) ? "selected" : ""} key={item}><input type="checkbox" checked={selectedEvidence.includes(item)} onChange={() => changeEvidence(item)} /><span><strong>{item}</strong><small>{instrument.sources[0]?.publisher ?? instrument.dataStatus}・資料截至 {instrument.dataAsOf}</small></span></label>)}</div></div>;
    }
    ...
    <section>
      ...
      <div className="decision-layout">
        ...
        <aside className="score-panel card" aria-live="polite">
          <span className="card-label">雙重評分・Demo 評估</span><h2>兩個問題，分開回答</h2><div className="score-progress"><span>決策驗證完成度</span><strong>{completedCount}/{steps.length}</strong><i><b style={{ width: `${(completedCount / steps.length) * 100}%` }} /></i></div><div className="score-card"><div><span>標的成立度</span><strong>{scored ? data.thesis.validityScore : "—"}</strong></div><p>{scored ? "標的本身是否具備值得持續研究的條件。" : "完成五個步驟並更新評分後顯示。"}</p></div><div className="score-card"><div><span>個人適合度</span><strong>{scored ? data.thesis.suitabilityScore : "—"}</strong></div><p>{scored ? "標的與你的目標、輪廓及規劃有何關聯。" : "個人條件與標的評估仍會分開處理。"}</p></div><p className="score-note">分數為前端 Demo 評估，不是官方資料或投資推薦；兩個分數不可合併。</p><Button full disabled={!scored} onClick={() => navigate("thesis")}>{scored ? "建立我的論點卡 →" : "完成驗證後建立論點卡"}</Button>
        </aside>
      </div>
    </section>
  );
