...
export function DecisionPage({ data }: { data: InvestmentData }) {
  ...
  const renderStep = () => {
    if (activeStep === 0) {
      return <div className="step-panel" role="tabpanel" aria-label="投資理由"><span className="card-label">步驟 1｜我的投資理由</span><h2>你為什麼認為這個標的值得關注？</h2><p>請先用自己的話說明，AI 只能協助整理，不能替你產生立場。</p><label className="field-label" htmlFor="reason">投資理由</label><textarea id="reason" value={reason} onChange={(event) => changeReason(event.target.value)} /><div className="field-status"><span>{reason.trim().length} 字</span><strong className={completed[0] ? "complete" : ""}>{completed[0] ? "✓ 已達最低完整度" : "至少輸入 20 個字"}</strong></div></div>;
    }

    if (activeStep === 1) {
      return <div className="step-panel" role="tabpanel" aria-label="支持證據"><span className="card-label">步驟 2｜支持證據</span><h2>哪些證據支持你的投資理由？</h2><p>以下承接標的分析頁的官方公開資料。請至少選擇一項真正與理由相關的證據。</p><div className="selection-list">{instrument.evidence.map((item) => <label className={selectedEvidence.includes(item) ? "selected" : ""} key={item}><input type="checkbox" checked={selectedEvidence.includes(item)} onChange={() => changeEvidence(item)} /><span><strong>{item}</strong><small>{instrument.sources[0]?.publisher ?? instrument.dataStatus}・資料截至 {instrument.dataAsOf}</small></span></label>)}</div></div>;
    }

    if (activeStep === 2) {
      return <div className="step-panel" role="tabpanel" aria-label="不同觀點"><span className="card-label">步驟 3｜不同觀點</span><h2>哪些反方觀點需要納入判斷？</h2><p>不是為了否定投資理由，而是避免只保留支持自己的資訊。</p><div className="selection-list">{instrument.counterEvidence.map((item) => <label className={selectedCounterEvidence.includes(item) ? "selected" : ""} key={item}><input type="checkbox" checked={selectedCounterEvidence.includes(item)} onChange={() => changeCounterEvidence(item)} /><span><strong>{item}</strong><small>選取代表已納入後續評估</small></span></label>)}</div></div>;
    }

    if (activeStep === 3) {
      return <div className="step-panel" role="tabpanel" aria-label="關鍵假設"><span className="card-label">步驟 4｜關鍵假設</span><h2>你的理由成立，需要哪些條件持續存在？</h2><p>關鍵假設會成為心跳追蹤的基礎。請至少選擇一項。</p><div className="selection-list">{instrument.assumptions.map((item) => <label className={selectedAssumptions.includes(item) ? "selected" : ""} key={item}><input type="checkbox" checked={selectedAssumptions.includes(item)} onChange={() => changeAssumption(item)} /><span><strong>{item}</strong><small>未來發生變化時重新檢視</small></span></label>)}</div></div>;
    }
