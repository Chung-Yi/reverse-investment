...
export function App() {
  ...
  if (error) content = <div className="feedback-state error"><h1>目前無法載入投資研究資料</h1><p>{error}</p></div>;
    ...
    const pages: Record<RouteId, React.ReactNode> = {
      ...
      tracking: <TrackingPage data={personalizedData} trackingRepository={mockTrackingRepository} conditionRepository={localTrackingConditionRepository} eventRepository={mockRelatedEventRepository} onOpenEvent={openRelatedEvent} feedbackInvitationVisible={!feedbackPromptDismissed && !feedbackSubmitted} onOpenFeedback={() => setFeedbackOpen(true)} onDismissFeedback={() => setFeedbackPromptDismissed(true)} />,
